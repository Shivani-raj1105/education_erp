...
function SportsActivityForm({ defaultValues, onSubmit, onCancel, isSaving }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Sport Name" required error={errors.sportName?.message}>
          <input
        ...
        <Field label="Position / Medal (optional)" error={errors.positionMedal?.message}>
          <input
...
export default function SportsActivities() {
  ...
  const createMutation = useCreateSportsActivity({
  ...
  const deleteMutation = useDeleteSportsActivity({
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
        <SearchBar
          ...
          placeholder="Search by name, USN, sport, level..."
          className="flex-1"
        />
        <button
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={COLS.length} className="text-center py-16 text-gray-400 dark:text-gray-500">
                    {search ? `No records found for "${search}"` : 'No sports activities added yet.'}
                  </td>
                </tr>
              ) : (
                records.map((rec) => (
                  <tr key={rec.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 whitespace-nowrap">
                        {rec.competitionLevel}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                      {rec.positionMedal ? (
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">
                          {rec.positionMedal}
                        </span>
                      ) : (
      ...
      <Modal
        ...
        title={editRecord ? 'Edit Sports Activity' : 'Add Sports Activity'}
        size="lg"
      >
        <SportsActivityForm
      ...
      <ConfirmDialog
        ...
        title="Delete Sports Activity"
        message={`Are you sure you want to delete the record for "${deleteTarget?.studentName} — ${deleteTarget?.sportName}"? This action cannot be undone.`}
        confirmLabel="Delete"
        variant="danger"
      />
    </div>
