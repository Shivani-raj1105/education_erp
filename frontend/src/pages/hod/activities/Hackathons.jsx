...
function HackathonForm({ defaultValues, onSubmit, onCancel, isSaving }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Hackathon Name" required error={errors.hackathonName?.message}>
          <input
            {...register('hackathonName', { required: 'Required', maxLength: { value: 200, message: 'Too long' } })}
            className={clsx('input-field', errors.hackathonName && 'border-red-400')}
            placeholder="e.g. Smart India Hackathon 2024"
          />
        </Field>
        <Field label="Position" required error={errors.position?.message}>
          <input
            {...register('position', { required: 'Required', maxLength: { value: 100, message: 'Too long' } })}
            className={clsx('input-field', errors.position && 'border-red-400')}
            placeholder="e.g. 1st Place, Runner Up, Finalist"
          />
        </Field>
        <Field label="Year" required error={errors.year?.message}>
          <select
            {...register('year', { required: 'Required', valueAsNumber: true })}
            className={clsx('input-field', errors.year && 'border-red-400')}
          >
...
export default function Hackathons() {
  ...
  const { data, isLoading, isFetching } = useHackathonList(queryParams);
  const records    = data?.data       || [];
  const pagination = data?.pagination || {};

  const createMutation = useCreateHackathon({ onSuccess: () => setModalOpen(false) });
  const updateMutation = useUpdateHackathon(editRecord?.id, {
    onSuccess: () => { setModalOpen(false); setEditRecord(null); },
  });
  const deleteMutation = useDeleteHackathon({ onSuccess: () => setDeleteTarget(null) });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Hackathons</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
        <SearchBar
          ...
          placeholder="Search by name, USN, hackathon, position..."
          className="flex-1"
        />
        <button
          onClick={() => qc.invalidateQueries({ queryKey: ['hackathons'] })}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={COLS.length} className="text-center py-16 text-gray-400 dark:text-gray-500">
                    {search ? `No records found for "${search}"` : 'No hackathon records added yet.'}
                  </td>
                </tr>
              ) : (
                records.map((rec) => (
                  <tr key={rec.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white max-w-[200px] truncate" title={rec.hackathonName}>{rec.hackathonName}</td>
                    <td className="px-4 py-3">
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 whitespace-nowrap">
                        {rec.position}
                      </span>
                    </td>
      ...
      <Modal
        ...
        title={editRecord ? 'Edit Hackathon Record' : 'Add Hackathon Record'}
        size="md"
      >
        <HackathonForm
      ...
      <ConfirmDialog
        ...
        title="Delete Record"
        message={`Delete hackathon record for "${deleteTarget?.studentName} — ${deleteTarget?.hackathonName}"?`}
        confirmLabel="Delete"
        variant="danger"
      />
    </div>
