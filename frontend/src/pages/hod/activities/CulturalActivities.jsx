...
const COLS = [
  ...
  { key: 'culturalActivityName', label: 'Activity' },
  { key: 'eventName',            label: 'Event Name' },
  { key: null,                   label: 'Position / Prize' },
...
function CulturalActivityForm({ defaultValues, onSubmit, onCancel, isSaving }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Cultural Activity Name" required error={errors.culturalActivityName?.message}>
          <input
            {...register('culturalActivityName', { required: 'Required', maxLength: { value: 150, message: 'Too long' } })}
            className={clsx('input-field', errors.culturalActivityName && 'border-red-400')}
            placeholder="e.g. Classical Dance, Singing"
          />
        </Field>

        <Field label="Event Name" required error={errors.eventName?.message}>
          <input
            {...register('eventName', { required: 'Required', maxLength: { value: 200, message: 'Too long' } })}
            className={clsx('input-field', errors.eventName && 'border-red-400')}
            placeholder="e.g. Annual Cultural Fest 2024"
          />
        </Field>

        <Field label="Position / Prize (optional)" error={errors.positionPrize?.message}>
          <input
            {...register('positionPrize')}
            className="input-field"
            placeholder="e.g. 1st Prize, Best Performer"
          />
        </Field>
...
export default function CulturalActivities() {
  ...
  const { data, isLoading, isFetching } = useCulturalActivityList(queryParams);
  const records    = data?.data       || [];
  const pagination = data?.pagination || {};

  const createMutation = useCreateCulturalActivity({
    onSuccess: () => { setModalOpen(false); },
  });

  const updateMutation = useUpdateCulturalActivity(editRecord?.id, {
    onSuccess: () => { setModalOpen(false); setEditRecord(null); },
  });

  const deleteMutation = useDeleteCulturalActivity({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Cultural Activities</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
        <SearchBar
          ...
          placeholder="Search by name, USN, activity, event..."
          className="flex-1"
        />
        <button
          onClick={() => qc.invalidateQueries({ queryKey: ['culturalActivities'] })}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={COLS.length} className="text-center py-16 text-gray-400 dark:text-gray-500">
                    {search ? `No records found for "${search}"` : 'No cultural activities added yet.'}
                  </td>
                </tr>
              ) : (
                records.map((rec) => (
                  <tr key={rec.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">{rec.culturalActivityName}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300 max-w-[160px] truncate" title={rec.eventName}>{rec.eventName}</td>
                    <td className="px-4 py-3 text-sm">
                      {rec.positionPrize ? (
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 whitespace-nowrap">
                          {rec.positionPrize}
                        </span>
                      ) : (
      ...
      <Modal
        ...
        title={editRecord ? 'Edit Cultural Activity' : 'Add Cultural Activity'}
        size="lg"
      >
        <CulturalActivityForm
      ...
      <ConfirmDialog
        ...
        title="Delete Cultural Activity"
        message={`Are you sure you want to delete the record for "${deleteTarget?.studentName} — ${deleteTarget?.eventName}"? This action cannot be undone.`}
        confirmLabel="Delete"
        variant="danger"
      />
    </div>
